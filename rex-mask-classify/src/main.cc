...
#define MODEL_NAME	"/mask.rknn" // mask detection rknn model path
#define LABELS		"/mask.txt" // mask label txt path
...
{	
	...
	ssd_init(MODEL_NAME,LABELS , &ctx);
	printf("init rknn complete \n");	
	while(1){
		rga_set_rect(&r_src.rect, 0, 0, CAM_W, CAM_H, CAM_W, CAM_H, RK_FORMAT_YCrCb_420_P);
		// rezie image input size(224,224,3)
		//(48,48,1)
		rga_set_rect(&r_dst.rect, 0, 0, 50, 50, 50, 50, RK_FORMAT_RGB_888); //mask detect input shape(50,50,1)
		ret = c_RkRgaBlit(&r_src, &r_dst, NULL);

		ssd_run(&ctx, (uint8_t*)dst_v, 50, 50, 50*50*1); // 50 * 50 * 1 = 2304
	}
