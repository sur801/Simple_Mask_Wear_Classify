...
mask_color=[(52, 97, 235),(250, 2, 2),(250, 171, 2),(255, 255, 255)]
...
if __name__ == '__main__':
    ...
    print('--> Loading model')
    rknn.load_rknn('mask_inc.rknn')

                         #(tf_pb='./test_eval_2.pb',
             #inputs=['FeatureExtractor/MobilenetV1/MobilenetV1/Conv2d_0/Conv2D'],
            # outputs=['concat_3','concat_2'],
    ...
    # Export RKNN Model
    #rknn.export_rknn('./Mask_1_1.rknn')

    # Direct Load RKNN Model
    # rknn.load_rknn('./ssd_mobilenet_v1_coco.rknn')

    # Set inputs
    orig_img = cv2.imread('./1430.jpg')
    ...
    vaildCnt = 0
    print(outputs[0][0][30][0][0])
    print(outputs[0][0][30][0][1])
    print(outputs[0][0][30][0][2])
    print(outputs[0][0][30][0][3])

    print(outputs[1][0][30][0])
    ...
    for i in range(0, vaildCnt):
        ...
        print("%d @ (%d, %d) (%d, %d) score=%f" % (topClassScoreIndex, xmin, ymin, xmax, ymax, expit(outputClasses[0][n][0]) ))
        print("%f %f %f" %(expit(outputClasses[0][n][0]),expit(outputClasses[0][n][1]),expit(outputClasses[0][n][2])) )
